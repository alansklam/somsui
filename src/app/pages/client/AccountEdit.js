...
const AccountEdit = () => {
  ...
  useEffect(() => {
    if (user.name !== undefined) {
      ...
      setBackupContact(user.mobile_phone_cn)
  ...
  const accountUpdateSubmit = () => {
    ...
    dispatch(updateAccount(data)).then(() => {
      onNotification({
        title: 'success',
        message: 'common.no-update-account-success',
        visible: true,
  ...
  const submitHandler = () => {
    ...
    if (name.length > 100) {
      onNotification({
        title: 'warning',
        message: 'common.no-input-name-length',
        visible: true,
        status: Math.floor(Math.random() * 100000),
      })
      return
    ...
    if (contact === '') {
      onNotification({
        title: 'warning',
        message: 'common.no-input-contact',
        visible: true,
        status: Math.floor(Math.random() * 100000),
      })
      return
      ...
      if (__result == null && __length <= 11 && __length >= 8) {
        ...
        onNotification({
          title: 'warning',
          message: 'common.no-input-contact-validate',
          visible: true,
          status: Math.floor(Math.random() * 100000),
        })
        return
    ...
    if (backupContact !== '' && backupContact !== null) {
      let __backupContact = backupContact
      let __re = /[^0-9]+/g
      let __result = __backupContact.match(__re)
      let __length = __backupContact.length
      if (__result == null && __length <= 11 && __length >= 8) {
      } else {
        onNotification({
          title: 'warning',
          message: 'common.no-input-backupContact-validate',
          visible: true,
          status: Math.floor(Math.random() * 100000),
        })
        return
    ...
    if (user.university_id && studentId === '') {
      onNotification({
        title: 'warning',
        message: 'common.no-input-studentID',
        visible: true,
        status: Math.floor(Math.random() * 100000),
      })
      return
    ...
    if (studentId.length > 50) {
      onNotification({
        title: 'warning',
        message: 'common.no-input-studentID-validate',
        visible: true,
        status: Math.floor(Math.random() * 100000),
      })
      return
    ...
    if (wechatId !== '' && wechatId.length > 50) {
      onNotification({
        title: 'warning',
        message: 'common.no-input-wechatID-validate',
        visible: true,
        status: Math.floor(Math.random() * 100000),
      })
      return
    ...
    if (currentPassword === '') {
      if (newPassword === '' && confirmPassword === '') {
        ...
        onNotification({
          title: 'warning',
          message: 'common.no-input-password-validate',
          visible: true,
          status: Math.floor(Math.random() * 100000),
        })
        return
      ...
      if (newPassword.length > 16 || newPassword.length < 8) {
        onNotification({
          title: 'warning',
          message: 'common.no-input-password-length',
          visible: true,
          status: Math.floor(Math.random() * 100000),
        })
        return
      ...
      if (newPassword === '' && confirmPassword === '') {
        onNotification({
          title: 'warning',
          message: 'common.no-input-empty-password-validate',
          visible: true,
          status: Math.floor(Math.random() * 100000),
        })
        return
        ...
        onNotification({
          title: 'warning',
          message: 'common.no-input-new-password-validate',
          visible: true,
          status: Math.floor(Math.random() * 100000),
        })
        return
        ...
        changePasswordApi(passwordData).then((res) => {
          if (res.data.status === 'success') {
            accountUpdateSubmit()
          } else if (res.data.status === 'error') {
            onNotification({
              title: 'warning',
              message: 'common.no-input-current-password-validate',
              visible: true,
              status: Math.floor(Math.random() * 100000),
            })
            return
    ...
    <>
      ...
      <div className='content-container'>
          ...
          <div className='mt-[20px] mx-[-15px]'>
            ...
            <Grid container className=''>
              ...
              <Grid item xs={12} sm={12} md={6} className='p-[15px]'>
                <CssTextField
                  fullWidth
                  id='standard-required-backupContact'
                  label={t('common.wd-backupContact')}
                  variant='standard'
                  value={backupContact}
                  onChange={(e) => {
                    let __backupContact = e.target.value
                    setBackupContact(__backupContact)
                  }}
                />
              </Grid>
            </Grid>
        ...
        <div className='flex item-center mt-[30px]'>
          <span className='custom-btn hand' onClick={submitHandler}>
            {t('common.wd-change')}
          </span>
        </div>
      </div>
