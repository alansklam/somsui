...
const ContentPage = (props) => {
    ...
    if (initial) {
      getProducts()
        .then((res) => {
          ...
          getStoragePeriodPrice(4, __products, __materials)
        })
        .catch((err) => {})
        .finally(() => {})
    }
  ...
  return (
    <div className='h-[100%]'>
      ...
      />
      {step === StepType.STOREITEM && (
        <ContentPage1
          onNotification={showNotification}
          onChangeStep={onChangeHandle}
          onRefreshCart={props.onRefreshCart}
          items={products}
          selectedItems={props.stores}
          getStoragePeriodPrice={getStoragePeriodPrice}
          storage_month={props.storage_month}
        />
      )}
      {step === StepType.MATERIALITEM && (
        <ContentPage2
          onNotification={showNotification}
          onChangeStep={onChangeHandle}
          onRefreshCart={props.onRefreshCart}
          items={materials}
          selectedItems={props.cartMaterials}
        />
      )}
      {step === StepType.STUFF && (
        <ContentPage3
      ...
      {step === StepType.PAYMENT && logged === 0 && (
        <ContentPage4
          onNotification={showNotification}
          onChangeStep={onChangeHandle}
          universities={universities}
      ...
      {step === StepType.PAYMENT && logged === 1 && (
        <ContentPage5
          onNotification={showNotification}
          onChangeStep={onChangeHandle}
          cartInfo={cartInfo}
      ...
      {step === StepType.SUCCESS && logged === 1 && (
        <ContentPage6
          onNotification={showNotification}
          onChangeStep={onChangeHandle}
          order={order}
